Ext.define('EAM.custom.AddButtonOnFocus', {
    ...
    getSelectors: function () {
        var self = this;
		var equipmentScreenCategories =[];
		var equipmentScreenClass =[];
		var equipmentDepartment =[];
		var loanedDepartment =[];
		var equipmentMeterunit =[];
		var person =[];
		var equipSysLevel =[];
		var equipAssLevel =[];
		var equipCompLevel =[];
        ...
        return {
			...
			'[extensibleFramework] [tabName=HDR][isTabView=true] [name=meterunit]': {
				customonblur: function(a,b){
					debugger;
					var panel = EAM.Utils.getCurrentTab().getFormPanel();
					var vals = panel.getFldValues(["organization"]);
						
					try{
						//-- calling the grid with Ajax
						var resp = EAM.Ajax.request({
							...
							params:{
								...
								LOV_ALIAS_VALUE_1: vals.organization,
						...
						let numericCnt = parseInt(cnt);
						if (!isNaN(numericCnt) && numericCnt > 0) {
							records = resp.responseData.pageData.grid.GRIDRESULT.GRID.DATA;
							equipmentMeterunit = records;
						}
					}catch(e){}
				}
			},
			'[extensibleFramework] [tabName=HDR][isTabView=true] [name=assignedto]': {
				customonblur: function(a,b){
					debugger;
					var panel = EAM.Utils.getCurrentTab().getFormPanel();
					var vals = panel.getFldValues(["organization"]);
					...
					try{
						...
						var resp = EAM.Ajax.request({
							...
							params:{
								GRID_NAME: "LVPERS",
								REQUEST_TYPE: "LIST.HEAD_DATA.STORED",
								LOV_ALIAS_NAME_1: "parameter.per_type",
								LOV_ALIAS_VALUE_1: "",
								LOV_ALIAS_TYPE_1: "text",
								
								LOV_ALIAS_NAME_2: "control.org",
								LOV_ALIAS_VALUE_2: vals.organization,
								LOV_ALIAS_TYPE_2: "text",
								
								LOV_ALIAS_NAME_3: "parameter.per_type",
								LOV_ALIAS_VALUE_3: "",
			...
			'[extensibleFramework] [tabName=HDR][isTabView=true] [name=syslevel]': {
				customonblur: function(a,b){
					debugger;
					var panel = EAM.Utils.getCurrentTab().getFormPanel();
					var vals = panel.getFldValues(["organization","class","classorganization"]);
					...
					try{
						...
						var resp = EAM.Ajax.request({
							...
							params:{
								...
								LOV_ALIAS_VALUE_2: vals.class,
								LOV_ALIAS_TYPE_2: "text",
								
								LOV_ALIAS_NAME_3: "parameter.classorg",
								LOV_ALIAS_VALUE_3: vals.classorganization,
								LOV_ALIAS_TYPE_3: "text"
							},
							async: false,
							method: "POST"
						...
						if (!isNaN(numericCnt) && numericCnt > 0) {
							records = resp.responseData.pageData.grid.GRIDRESULT.GRID.DATA;
							equipSysLevel = records;
						}
					}catch(e){}
				}
			},
			'[extensibleFramework] [tabName=HDR][isTabView=true] [name=asslevel]': {
				customonblur: function(a,b){
					debugger;
					var panel = EAM.Utils.getCurrentTab().getFormPanel();
					var vals = panel.getFldValues(["organization","class","classorganization","syslevel"]);
					...
					try{
						...
						var resp = EAM.Ajax.request({
							url: "GRIDDATA",
							params:{
								GRID_NAME: "LVASSLEVELCLSCNTXT",
								REQUEST_TYPE: "LIST.HEAD_DATA.STORED",
								LOV_ALIAS_NAME_1: "param.rentity",
								LOV_ALIAS_VALUE_1:"OBJ",
								LOV_ALIAS_TYPE_1: "text",
								
								LOV_ALIAS_NAME_2: "param.syslevel",
								LOV_ALIAS_VALUE_2: vals.syslevel,
								LOV_ALIAS_TYPE_2: "text",
								
								LOV_ALIAS_NAME_3: "parameter.class",
								LOV_ALIAS_VALUE_3: vals.class,
								LOV_ALIAS_TYPE_3: "text",
								
								LOV_ALIAS_NAME_4: "parameter.classorg",
								LOV_ALIAS_VALUE_4: vals.classorganization,
						...
						if (!isNaN(numericCnt) && numericCnt > 0) {
							records = resp.responseData.pageData.grid.GRIDRESULT.GRID.DATA;
							equipAssLevel = records;
						}
					}catch(e){}
				}
			},
			'[extensibleFramework] [tabName=HDR][isTabView=true] [name=complevel]': {
				//asslevel
				customonblur: function(a,b){
					debugger;
					var panel = EAM.Utils.getCurrentTab().getFormPanel();
					var vals = panel.getFldValues(["organization","class","classorganization","syslevel","asslevel"]);
					...
					try{
						...
						var resp = EAM.Ajax.request({
							...
							params:{
								...
								LOV_ALIAS_NAME_3: "param.asslevel",
								LOV_ALIAS_VALUE_3: vals.asslevel,
								LOV_ALIAS_TYPE_3: "text",
								
								LOV_ALIAS_NAME_4: "parameter.class",
								LOV_ALIAS_VALUE_4: vals.class,
								LOV_ALIAS_TYPE_4: "text",
								
								LOV_ALIAS_NAME_5: "parameter.classorg",
								LOV_ALIAS_VALUE_5: vals.classorganization,
						...
						if (!isNaN(numericCnt) && numericCnt > 0) {
							records = resp.responseData.pageData.grid.GRIDRESULT.GRID.DATA;
							equipCompLevel = records;
						}
					}catch(e){}
				}
			},
			'[extensibleFramework] [tabName=HDR][isTabView=true] [name=costcode]': {
				customonblur: function(a,b){
					debugger;
					var panel = EAM.Utils.getCurrentTab().getFormPanel();
					var vals = panel.getFldValues(["organization","flcust","fleetcustorg"]);
					...
					try{
						...
						var resp = EAM.Ajax.request({
							...
							params:{
								...
								LOV_ALIAS_NAME_2: "param.flcust",
								LOV_ALIAS_VALUE_2: vals.flcust,
								LOV_ALIAS_TYPE_2: "text",
								
								LOV_ALIAS_NAME_3: "param.fleetcustorg",
								LOV_ALIAS_VALUE_3: vals.fleetcustorg,
								LOV_ALIAS_TYPE_3: "text"
							},
							async: false,
							method: "POST"
			...
			'[extensibleFramework] [tabName=HDR][isTabView=true] [name=loanedtodepartment]': {
				customonblur: function(a,b){
					debugger;
					var panel = EAM.Utils.getCurrentTab().getFormPanel();
						var vals = panel.getFldValues(["organization"]);
						...
						try{
							...
							var resp = EAM.Ajax.request({
								...
								params:{
									GRID_NAME: "LVCLAS",
									REQUEST_TYPE: "LIST.HEAD_DATA.STORED",
									LOV_ALIAS_NAME_1: "parameter.rentity",
									LOV_ALIAS_VALUE_1: "OBJ",
									LOV_ALIAS_TYPE_1: "text",
    ...
    onPredictButtonClick: function (targetField) {
        ...
        const BASE_URL = 'https://cde8-102-191-169-222.ngrok-free.app'; // Ensure this is your correct backend URL
                ...
                if (titleIndex !== -1) {
                    var button = Ext.create('Ext.button.Button', {
                        ...
                        handler: function (button, e) { 
                            ...
                            const BASE_URL = 'https://cde8-102-191-169-222.ngrok-free.app'; // Ensure this is correct
